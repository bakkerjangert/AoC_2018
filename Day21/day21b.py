...
#######################################################
### RUN WITH PYPY FOR SPEED - PYTHON TAKES LONG!!!! ###
#######################################################
...
while True:
    ...
    line_no = ip_number + 2
    if line_no == 30:
        print(f'Current reg[3] = {registers[3]}')
        if registers[3] not in founds:
            last_found = registers[3]
            founds.add(registers[3])
        else:
            print(f'The answer is {last_found}')
            exit()

    # print(f'{registers} --> IP point {ip_number + 2}')
    # if counter % 10 == 0:
    #     breakpoint()
